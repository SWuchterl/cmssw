...
namespace l1ct {
  ...
  namespace Scales {
    ...
    inline float floatZ0(z0_t z0) {
      // std::cout<<"datatypes z0: "<<z0.to_float()<<" "<<z0.to_float() * Z0_LSB<<std::endl;
      return z0.to_float() * Z0_LSB; }
    inline float floatDxy(dxy_t dxy) {
      // std::cout<<"datatypes: "<<dxy.to_float()<<" "<<dxy.to_float() * DXYSQRT_LSB<<std::endl;
      return dxy.to_float() * DXYSQRT_LSB;
      }
