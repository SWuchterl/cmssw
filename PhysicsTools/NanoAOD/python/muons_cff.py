...
muonParTTrainVariables = cms.EDProducer(
    ...
    pfVars=cms.PSet(
        ...
        # PF_dxy_asinh=cms.string("?hasTrackDetails?log(abs(bestTrack().dxy()+sqrt(bestTrack().dxy()*bestTrack().dxy() + 1+1.e-8))):0"),
        # PF_dxysig=cms.string("?hasTrackDetails?bestTrack().dxy()/bestTrack().dxyError():0"),
        # PF_dxysig_asinh=cms.string("?hasTrackDetails?log(abs(bestTrack().dxy()/bestTrack().dxyError()+sqrt((bestTrack().dxy()/bestTrack().dxyError())*(bestTrack().dxy()/bestTrack().dxyError()) + 1+1.e-8))):0"),
        # PF_dz=cms.string("?hasTrackDetails?bestTrack().dz():0"),
        # PF_dz_asinh=cms.string("?hasTrackDetails?log(abs(bestTrack().dz()+sqrt(bestTrack().dz()*bestTrack().dz() + 1+1.e-8))):0"),
        # PF_dzsig=cms.string("?hasTrackDetails?bestTrack().dz()/bestTrack().dzError():0"),
        # PF_dzsig_asinh=cms.string("?hasTrackDetails?log(abs(bestTrack().dz()/bestTrack().dzError()+sqrt((bestTrack().dz()/bestTrack().dzError())*(bestTrack().dz()/bestTrack().dzError()) + 1+1.e-8))):0"),
    ...
    ltVars=cms.PSet(
        ...
        # LT_dxy_asinh=cms.string("?hasTrackDetails?log(abs(bestTrack().dxy()+sqrt(bestTrack().dxy()*bestTrack().dxy() + 1+1.e-8))):0"),
        # LT_dxysig=cms.string("?hasTrackDetails?bestTrack().dxy()/bestTrack().dxyError():0"),
        # LT_dxysig_asinh=cms.string("?hasTrackDetails?log(abs(bestTrack().dxy()/bestTrack().dxyError()+sqrt((bestTrack().dxy()/bestTrack().dxyError())*(bestTrack().dxy()/bestTrack().dxyError()) + 1+1.e-8))):0"),
        # LT_dz=cms.string("?hasTrackDetails?bestTrack().dz():0"),
        # LT_dz_asinh=cms.string("?hasTrackDetails?log(abs(bestTrack().dz()+sqrt(bestTrack().dz()*bestTrack().dz() + 1+1.e-8))):0"),
        # LT_dzsig=cms.string("?hasTrackDetails?bestTrack().dz()/bestTrack().dzError():0"),
        # LT_dzsig_asinh=cms.string("?hasTrackDetails?log(abs(bestTrack().dz()/bestTrack().dzError()+sqrt((bestTrack().dz()/bestTrack().dzError())*(bestTrack().dz()/bestTrack().dzError()) + 1+1.e-8))):0"),
...
muonTable = simplePATMuonFlatTableProducer.clone(
    ...
    doc=cms.string("slimmedMuons after basic selection (" +
                   finalMuons.cut.value()+")"),
    variables=cms.PSet(CandVars,
                       ptErr=Var("bestTrack().ptError()", float,
                                 doc="ptError of the muon track", precision=6),
                       tunepRelPt=Var("tunePMuonBestTrack().pt/pt", float,
                                      doc="TuneP relative pt, tunePpt/pt", precision=6),
                       tuneP_pterr=Var("tunePMuonBestTrack().ptError()", float,
                                       doc="pTerr from tunePMuonBestTrack", precision=6),
                       tuneP_charge=Var("? tunePMuonBestTrack().isNonnull() && tunePMuonBestTrack().isAvailable() ? tunePMuonBestTrack().charge(): -99",
                                        float, doc="tunePMuonBestTrack() charge", precision=6),
                       dz=Var(
                           "dB('PVDZ')", float, doc="dz (with sign) wrt first PV, in cm", precision=10),
                       dzErr=Var("abs(edB('PVDZ'))", float,
                                 doc="dz uncertainty, in cm", precision=6),
                       dxybs=Var(
                           "dB('BS2D')", float, doc="dxy (with sign) wrt the beam spot, in cm", precision=10),
                       dxybsErr=Var("edB('BS2D')", float,
                                    doc="dxy uncertainty wrt the beam spot, in cm", precision=6),
                       ...
                       sip3d=Var("abs(dB('PV3D')/edB('PV3D'))", float,
                                 doc="3D impact parameter significance wrt first PV", precision=10),
                       # keep higher precision since people have cuts with 3 digits on this
                       segmentComp=Var("segmentCompatibility()", float,
                                       doc="muon segment compatibility", precision=14),
                       nStations=Var("numberOfMatchedStations", "uint8",
                                     doc="number of matched stations with default arbitration (segment & track)"),
                       nTrackerLayers=Var("?track.isNonnull?innerTrack().hitPattern().trackerLayersWithMeasurement():0",
                                          "uint8", doc="number of layers in the tracker"),
                       bestTrackType=Var("muonBestTrackType()", "uint8",
                                         doc="Type of track used (1=inner, 2=STA, 3=global, 4=TPFMS, 5=Picky, 6=DYT)"),
                       highPurity=Var("?track.isNonnull?innerTrack().quality('highPurity'):0",
                                      bool, doc="inner track is high purity"),
                       ...
                       tkRelIso=Var("isolationR03().sumPt/pt", float,
                                    doc="Tracker-based relative isolation dR=0.3 for highPt, trkIso/pt", precision=6),
                       ...
                       pfRelIso03_chg=Var("pfIsolationR03().sumChargedHadronPt/pt",
                                          float, doc="PF relative isolation dR=0.3, charged component"),
                       pfRelIso03_all=Var("(pfIsolationR03().sumChargedHadronPt + max(pfIsolationR03().sumNeutralHadronEt + pfIsolationR03().sumPhotonEt - pfIsolationR03().sumPUPt/2,0.0))/pt",
                                          float, doc="PF relative isolation dR=0.3, total (deltaBeta corrections)"),
                       pfRelIso04_all=Var("(pfIsolationR04().sumChargedHadronPt + max(pfIsolationR04().sumNeutralHadronEt + pfIsolationR04().sumPhotonEt - pfIsolationR04().sumPUPt/2,0.0))/pt",
                                          float, doc="PF relative isolation dR=0.4, total (deltaBeta corrections)"),
                       ...
                       tightCharge=Var("?(muonBestTrack().ptError()/muonBestTrack().pt() < 0.2)?2:0", "uint8",
                                       doc="Tight charge criterion using pterr/pt of muonBestTrack (0:fail, 2:pass)"),
                       looseId=Var("passed('CutBasedIdLoose')",
                                   bool, doc="muon is loose muon"),
                       isPFcand=Var("isPFMuon", bool,
                                    doc="muon is PF candidate"),
                       isGlobal=Var("isGlobalMuon", bool,
                                    doc="muon is global muon"),
                       isTracker=Var("isTrackerMuon", bool,
                                     doc="muon is tracker muon"),
                       isStandalone=Var("isStandAloneMuon", bool,
                                        doc="muon is a standalone muon"),
                       mediumId=Var("passed('CutBasedIdMedium')", bool,
                                    doc="cut-based ID, medium WP"),
                       mediumPromptId=Var("passed('CutBasedIdMediumPrompt')",
                                          bool, doc="cut-based ID, medium prompt WP"),
                       tightId=Var("passed('CutBasedIdTight')", bool,
                                   doc="cut-based ID, tight WP"),
                       softId=Var("passed('SoftCutBasedId')",
                                  bool, doc="soft cut-based ID"),
                       softMvaId=Var("passed('SoftMvaId')",
                                     bool, doc="soft MVA ID"),
                       softMva=Var("softMvaValue()", float,
                                   doc="soft MVA ID score", precision=6),
                       softMvaRun3=Var("softMvaRun3Value()", float,
                                       doc="soft MVA Run3 ID score", precision=6),
                       highPtId=Var("?passed('CutBasedIdGlobalHighPt')?2:passed('CutBasedIdTrkHighPt')", "uint8",
                                    doc="high-pT cut-based ID (1 = tracker high pT, 2 = global high pT, which includes tracker high pT)"),
                       pfIsoId=Var("passed('PFIsoVeryLoose')+passed('PFIsoLoose')+passed('PFIsoMedium')+passed('PFIsoTight')+passed('PFIsoVeryTight')+passed('PFIsoVeryVeryTight')",
                                   "uint8", doc="PFIso ID from miniAOD selector (1=PFIsoVeryLoose, 2=PFIsoLoose, 3=PFIsoMedium, 4=PFIsoTight, 5=PFIsoVeryTight, 6=PFIsoVeryVeryTight)"),
                       tkIsoId=Var("?passed('TkIsoTight')?2:passed('TkIsoLoose')",
                                   "uint8", doc="TkIso ID (1=TkIsoLoose, 2=TkIsoTight)"),
                       miniIsoId=Var("passed('MiniIsoLoose')+passed('MiniIsoMedium')+passed('MiniIsoTight')+passed('MiniIsoVeryTight')", "uint8",
                                     doc="MiniIso ID from miniAOD selector (1=MiniIsoLoose, 2=MiniIsoMedium, 3=MiniIsoTight, 4=MiniIsoVeryTight)"),
                       mvaMuID=Var("userFloat('mvaIDMuon')", float,
                                   doc="MVA-based ID score", precision=6),
                       mvaMuID_WP=Var("userFloat('mvaIDMuon_wpMedium') + userFloat('mvaIDMuon_wpTight')",
                                      "uint8", doc="MVA-based ID selector WPs (1=MVAIDwpMedium,2=MVAIDwpTight)"),
                       multiIsoId=Var("?passed('MultiIsoMedium')?2:passed('MultiIsoLoose')", "uint8",
                                      doc="MultiIsoId from miniAOD selector (1=MultiIsoLoose, 2=MultiIsoMedium)"),
                       puppiIsoId=Var("passed('PuppiIsoLoose')+passed('PuppiIsoMedium')+passed('PuppiIsoTight')",
                                      "uint8", doc="PuppiIsoId from miniAOD selector (1=Loose, 2=Medium, 3=Tight)"),
                       triggerIdLoose=Var("passed('TriggerIdLoose')",
                                          bool, doc="TriggerIdLoose ID"),
                       inTimeMuon=Var("passed('InTimeMuon')",
                                      bool, doc="inTimeMuon ID"),
                       ...
                       VXBS_Cov00=Var("? tunePMuonBestTrack().isNonnull() && tunePMuonBestTrack().isAvailable() ? tunePMuonBestTrack().covariance(0,0) : -999",
                                      float, doc="0, 0 element of the VXBS Covariance matrix", precision=16),
                       VXBS_Cov03=Var("? tunePMuonBestTrack().isNonnull() && tunePMuonBestTrack().isAvailable() ? tunePMuonBestTrack().covariance(0,3) : -999",
                                      float, doc="0, 3 element of the VXBS Covariance matrix", precision=16),
                       VXBS_Cov33=Var("? tunePMuonBestTrack().isNonnull() && tunePMuonBestTrack().isAvailable() ? tunePMuonBestTrack().covariance(3,3) : -999",
                                      float, doc="3, 3 element of the VXBS Covariance matrix", precision=16),
                       ),
    externalVariables=cms.PSet(
        promptMVA=ExtVar(cms.InputTag("muonPROMPTMVA"), float,
                         doc="Prompt MVA lepton ID score. Corresponds to the previous mvaTTH", precision=14),
        mvaLowPt=ExtVar(cms.InputTag("muonMVALowPt"), float,
                        doc="Low pt muon ID score", precision=14),
        pnScore_prompt=ExtVar(cms.InputTag("muonPNetScores:prompt"), float,
                              doc="PNet muon ID score for lepton from W/Z/H bosons", precision=14),
        pnScore_heavy=ExtVar(cms.InputTag("muonPNetScores:heavy"), float,
                             doc="PNet muon ID score for lepton from B or D hadrons", precision=14),
        pnScore_light=ExtVar(cms.InputTag("muonPNetScores:light"), float,
                             doc="PNet muon ID score for lepton from hadrons w/o b or c quarks OR w/o generator matching", precision=14),
        pnScore_tau=ExtVar(cms.InputTag("muonPNetScores:tau"), float,
                           doc="PNet muon ID score for decay of tau to light leptons (mu)", precision=14),
        parTScore_prompt=ExtVar(cms.InputTag("muonParTScores:prompt"), float,
                                doc="ParT muon ID score for lepton from W/Z/H bosons", precision=14),
        parTScore_heavy=ExtVar(cms.InputTag("muonParTScores:heavy"), float,
                               doc="ParT muon ID score for lepton from B or D hadrons", precision=14),
        parTScore_light=ExtVar(cms.InputTag("muonParTScores:light"), float,
                               doc="ParT muon ID score for lepton from hadrons w/o b or c quarks OR w/o generator matching", precision=14),
        parTScore_tau=ExtVar(cms.InputTag("muonParTScores:tau"), float,
                             doc="ParT muon ID score for decay of tau to light leptons (mu)", precision=14),
        parTScore_fake=ExtVar(cms.InputTag("muonParTScores:fake"), float,
                              doc="ParT muon ID score for fake leptons", precision=14),
        fsrPhotonIdx=ExtVar(cms.InputTag("leptonFSRphotons:muFsrIndex"), "int16",
                            doc="Index of the lowest-dR/ET2 among associated FSR photons"),
        bsConstrainedPt=ExtVar(cms.InputTag("muonBSConstrain:muonBSConstrainedPt"),
                               float, doc="pT with beamspot constraint", precision=-1),
        bsConstrainedPtErr=ExtVar(cms.InputTag("muonBSConstrain:muonBSConstrainedPtErr"),
                                  float, doc="pT error with beamspot constraint ", precision=6),
        bsConstrainedChi2=ExtVar(cms.InputTag("muonBSConstrain:muonBSConstrainedChi2"),
                                 float, doc="chi2 of beamspot constraint", precision=6),
    ),
)
...
muonTask = cms.Task(slimmedMuonsUpdated, isoForMu, ptRatioRelForMu,
                    slimmedMuonsWithUserData, finalMuons, finalLooseMuons)
