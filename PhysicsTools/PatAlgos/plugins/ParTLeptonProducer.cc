...
class ParTLeptonProducer : public edm::stream::EDProducer<edm::GlobalCache<cms::Ort::ONNXRuntime>> {
public:
  explicit ParTLeptonProducer(const edm::ParameterSet &, const cms::Ort::ONNXRuntime *);
  ~ParTLeptonProducer() override {}
...
template <typename LeptonType>
ParTLeptonProducer<LeptonType>::ParTLeptonProducer(const edm::ParameterSet &iConfig, const cms::Ort::ONNXRuntime *cache)
...
template <typename LeptonType>
void ParTLeptonProducer<LeptonType>::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
...
template <typename LeptonType>
std::unique_ptr<cms::Ort::ONNXRuntime> ParTLeptonProducer<LeptonType>::initializeGlobalCache(
    const edm::ParameterSet &cfg) {
  std::cout << "Initializing ONNXRuntime with model: " << cfg.getParameter<edm::FileInPath>("model_path").fullPath() << std::endl;
  return std::make_unique<cms::Ort::ONNXRuntime>(cfg.getParameter<edm::FileInPath>("model_path").fullPath());
}

template <typename LeptonType>
void ParTLeptonProducer<LeptonType>::globalEndJob(const cms::Ort::ONNXRuntime *cache) {}

template <typename LeptonType>
void ParTLeptonProducer<LeptonType>::fillDescriptions(edm::ConfigurationDescriptions &descriptions) {
  ...
  desc.add<std::string>("preprocess_json", "PhysicsTools/NanoAOD/data/ParTMuonId/preprocess.json");
  desc.add<edm::FileInPath>("model_path", edm::FileInPath("PhysicsTools/NanoAOD/data/ParTMuonId/model.onnx"));
...
template <typename LeptonType>
void ParTLeptonProducer<LeptonType>::make_inputs(const DeepBoostedJetFeatures &taginfo) {
...
typedef ParTLeptonProducer<pat::Muon> MuonParTProducer;
typedef ParTLeptonProducer<pat::Electron> ElectronParTProducer;

//define this as a plug-in
DEFINE_FWK_MODULE(MuonParTProducer);
DEFINE_FWK_MODULE(ElectronParTProducer);
