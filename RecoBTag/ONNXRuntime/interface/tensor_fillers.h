...
namespace btagbtvdeep {
  ...
  template <class UparT_features>
  void ScoutingUparT_tensor_filler(cms::Ort::FloatArrays& data,
                           const ScoutingUparT::InputFeatures ifeature,
                           const std::vector<UparT_features>& features,
                           const unsigned int max_n,
                           const float*& start,
                           unsigned offset) {
    ...
    for (std::size_t n = 0; n < max_n; n++) {
      const auto& f = features.at(n);
      ptr = &data[ifeature][offset + n * ScoutingUparT::N_InputFeatures.at(ifeature)];
      start = ptr;
      const std::vector<float>& inputs = inputs_ScoutingUparT(f, ifeature);
      for (unsigned int i = 0; i < inputs.size(); i++) {
        *ptr = inputs[i];
        ++ptr;
      }
      if (!inputs.empty())
        --ptr;
      assert(start + ScoutingUparT::N_InputFeatures.at(ifeature) - 1 == ptr);
    }
  }

  template <class UparT_features>
  void ScoutingUparT_tensor_filler(std::vector<float>& vdata,
                           const ScoutingUparT::InputFeatures ifeature,
                           const std::vector<UparT_features>& features,
                           const unsigned int target_n) {
    unsigned int n = std::clamp(
